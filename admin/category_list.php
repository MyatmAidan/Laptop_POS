...
$res = selectData('category', $mysql, "", "*", "");
$delete_id = isset($_GET['delete_id']) ?  $_GET['delete_id'] : '';
if ($delete_id !== '') {
    $res = deleteData('category', $mysql, "category_id=$delete_id");
    if ($res) {
        $url = $admin_base_url . "category_list.php?success=Deleted Category Success";
        header("Location: $url");
    }
}
                                ...
                                <?php if ($res->num_rows > 0) {
                                    while ($row = $res->fetch_assoc()) { ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="<?= $admin_base_url . 'category_edit.php?id=' . $row['category_id'] ?>" class="btn btn-sm btn-primary"><i class="fa-solid fa-pen-to-square"></i></a>
                                                <button data-id="<?= $row['category_id'] ?>" class="btn btn-sm btn-danger delete_btn"><i class="fa-solid fa-trash"></i></button>
                                            </td>
                                        </tr>
                                <?php }
                                } ?>
