...
if (isset($_POST['form_sub']) && $_POST['form_sub'] == '1') {
    ...
    if ($name === '' || strlen($name) === 0) {
        $error = true;
        $name_error = "Please Fill Payment Method Name.";
    } else if (strlen($name) < 3) {
        $error = true;
        $name_error = "Method name must be fill greater then 3.";
    } else if (strlen($name) > 100) {
        $error = true;
        $name_error = "Method name must be fill less then 100.";
    }

    if (!$error) {
        // normalize to lowercase for checking
        $check_sql = "SELECT method_id FROM method WHERE LOWER(method_name) = LOWER('$name') LIMIT 1";
        ...
        if ($check_res && $check_res->num_rows > 0) {
            ...
            $result = insertData('method', $mysql, $data);

            if ($result) {
                $url = $admin_base_url . 'method_list.php?success=Created Success';
                header("Location: $url");
                exit;
            } else {
                $error = true;
                $error_message = "Payment Method Create Fail.";
            }
        }
    }
}
...
<div class="content-body">
    <div class="container-fluid">
        <div class="d-flex justify-content-between">
            ...
            <div class="">
                <a href="<?= $admin_base_url . 'method_list.php' ?>" class="btn btn-dark">
                    Back
                </a>
            </div>
        </div>
