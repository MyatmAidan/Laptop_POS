...
if (isset($_GET['id']) && $_GET['id'] !== '') {
    $id = $_GET['id'];
    $select_res = selectData('category', $mysql, "WHERE category_id='$id'");
    // var_dump('helrl');
    // die;
...
if (isset($_POST['form_sub']) && $_POST['form_sub'] == '1') {
    ...
    if (!$error) {
        ...
        $result = updateData('category', $mysql, $data, $where);
        if ($result) {
            $url = $admin_base_url . 'category_list.php?success=Updated Success';
            header("Location: $url");
            exit;
        } else {
            $error = true;
